...
function buildCubaListRico() {
  ...
  if (_catRicoDetalhe) {
    ...
    var _carCubaId = '_car_' + (_catRicoDetalhe.cuba.id||'x').replace(/[^a-z0-9]/gi,'_');
    var _carFotos  = [];
    var _carImg    = _cubaImg(_catRicoDetalhe.cuba);
    if (_carImg) _carFotos.push(_carImg);
    if (_catRicoDetalhe.cuba.fotos) _catRicoDetalhe.cuba.fotos.forEach(function(f){ if(f&&_carFotos.indexOf(f)===-1)_carFotos.push(f); });
    _carBindSwipe(_carCubaId, _carFotos.length);
    return;
  }
...
function _renderCardCuba(cuba, tipo, idx) {
  ...
  // Preço
  var pctCard = _descPct(cuba, meta);
  ...
  if (pctCard > 0) {
    h += '<div style="display:flex;align-items:center;gap:5px;margin-bottom:3px;">'
      + '<span style="font-size:.58rem;color:var(--t3);text-decoration:line-through;">R$\u00a0' + meta.pr_orig.toLocaleString('pt-BR') + '</span>'
      + '<span style="font-size:.58rem;font-weight:800;background:var(--gold2);color:#0f0c00;padding:1px 5px;border-radius:3px;">' + pctCard + '% OFF</span>'
      + '</div>';
  } else {
    h += '<div style="font-size:.56rem;color:var(--gold3);text-transform:uppercase;letter-spacing:.8px;margin-bottom:2px;">c/ instalação</div>';
  }
...
function _renderDetalhe(cuba, tipo, idx) {
  ...
  // ── Carrossel de fotos — swipe touch ─────────────────────
  var allFotos = [];
  if (img) allFotos.push(img);
  if (cuba.fotos && Array.isArray(cuba.fotos)) {
    cuba.fotos.forEach(function(f){ if (f && allFotos.indexOf(f) === -1) allFotos.push(f); });
  }
  var carId = '_car_' + (cuba.id||'x').replace(/[^a-z0-9]/gi,'_');
  var nFotos = allFotos.length;

  h += '<div style="position:relative;margin-bottom:16px;">';
  if (nFotos > 0) {
    // Trilho deslizante
    h += '<div id="'+carId+'_trilho" style="display:flex;overflow:hidden;border-radius:16px;'
      + 'background:var(--s2);aspect-ratio:4/3;touch-action:pan-y;">';
    allFotos.forEach(function(f, i){
      h += '<div style="flex:0 0 100%;width:100%;display:flex;align-items:center;justify-content:center;">'
        + '<img src="'+f+'" alt="'+_escHtml(cuba.nm)+'" '
        + 'style="width:100%;height:100%;object-fit:contain;padding:20px;box-sizing:border-box;" loading="'+(i===0?'eager':'lazy')+'">'
        + '</div>';
    });
    h += '</div>';

    // Badge + tipo_mat sobrepostos
    if (meta.badge) {
      h += '<div style="position:absolute;top:10px;left:10px;background:rgba(0,0,0,.72);'
        ...
        + 'color:' + (meta.destaque?'var(--gold2)':'#fff') + ';font-weight:700;pointer-events:none;">'
        + meta.badge + '</div>';
    }
    if (meta.tipo_mat) {
      h += '<div style="position:absolute;top:10px;right:10px;background:rgba(0,0,0,.55);'
        ...
        + 'font-weight:600;pointer-events:none;">' + meta.tipo_mat + '</div>';
    }
    ...
    if (nFotos > 1) {
      ...
      h += '<div id="'+carId+'_cnt" style="position:absolute;bottom:10px;right:10px;'
        ...
        + 'font-size:.6rem;color:#fff;pointer-events:none;">1/'+nFotos+'</div>';
      // Bolinhas
      ...
      allFotos.forEach(function(f, i){
        h += '<div style="width:'+(i===0?'18':'7')+'px;height:7px;border-radius:4px;transition:all .2s;'
          + 'background:'+(i===0?'var(--gold2)':'var(--bd2)')+';"></div>';
      });
      h += '</div>';
    }
  } else {
    h += '<div style="background:var(--s2);border-radius:16px;aspect-ratio:4/3;'
  ...
  // Thumbs clicáveis (se > 1 foto)
  if (nFotos > 1) {
    h += '<div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:4px;scrollbar-width:none;margin-bottom:14px;">';
    allFotos.forEach(function(f, i){
      h += '<div onclick="_carIrFoto(\''+carId+'\','+i+','+nFotos+');" '
        + 'id="'+carId+'_th'+i+'" '
        + 'style="flex-shrink:0;width:50px;height:50px;border-radius:8px;cursor:pointer;overflow:hidden;'
        + 'border:2px solid '+(i===0?'var(--gold2)':'var(--bd2)')+';background:var(--s2);transition:border-color .15s;">'
  ...
  if (pctDet > 0) {
    h += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;">'
      + '<span style="font-size:.72rem;color:var(--t3);text-decoration:line-through;">R$\u00a0' + meta.pr_orig.toLocaleString('pt-BR') + '</span>'
      + '<span style="font-size:.7rem;font-weight:800;background:var(--gold2);color:#0f0c00;padding:2px 8px;border-radius:4px;">' + pctDet + '% OFF</span>'
      + '</div>';
  } else {
  ...
  if (!pctDet) {
    // só mostra "instalação inclusa" quando não tem badge de desconto (evita repetição)
  }
  if (instStr) {
  ...
  // ── Botões de ação ───────────────────────────────────────
  // Botão principal: Comprar agora (dourado sólido, largura total)
  h += '<button onclick="_selecionarCubaDetalhe(\'' + _escHtml(cuba.id) + '\',\'' + tipo + '\');" '
    + 'style="width:100%;padding:15px;border-radius:12px;border:none;cursor:pointer;margin-bottom:8px;'
    + 'background:linear-gradient(135deg,var(--gold2),#a07828);'
    + 'color:#0f0c00;font-family:Outfit,sans-serif;font-size:.95rem;font-weight:800;letter-spacing:.3px;'
    + 'box-shadow:0 3px 14px rgba(201,168,76,.4);">🛒 Comprar agora</button>';
  // Linha secundária
  h += '<div style="display:flex;gap:8px;margin-bottom:12px;">';
  h += '<button onclick="_selecionarCubaDetalhe(\'' + _escHtml(cuba.id) + '\',\'' + tipo + '\');" '
    + 'style="flex:2;padding:12px;border-radius:12px;cursor:pointer;'
    + 'border:1.5px solid var(--gold3);background:transparent;'
    + 'color:var(--gold2);font-family:Outfit,sans-serif;font-size:.8rem;font-weight:700;">'
    + '+ Adicionar ao orçamento</button>';
  h += '<button onclick="_compartilharCubaDetalhe(\'' + _escHtml(cuba.id) + '\',\'' + tipo + '\');" '
    + 'style="flex:1;padding:12px;border-radius:12px;border:1px solid #1a4a2a;'
...
function _carAtualizarUI(carId, idx, total) {
  ...
  if (dots) {
    dots.querySelectorAll('div').forEach(function(d, i){
      d.style.background = i===idx ? 'var(--gold2)' : 'var(--bd2)';
      d.style.width      = i===idx ? '18px' : '7px';
    });
  }
  for (var i=0; i<total; i++) {
    var th = document.getElementById(carId+'_th'+i);
    if (th) th.style.borderColor = i===idx ? 'var(--gold2)' : 'var(--bd2)';
  }
}
