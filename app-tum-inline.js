...
var SEL = {
  ...
  // ── RODAPÉ + LAJE VEDANTE ──
  rebaixo: {
    avRodape:    5,     // avanço lateral do rodapé por lado (cm) → define área útil
    lajeVedante: false, // laje de concreto vedante abaixo das tampas
    lajeInteira: true,  // true = 1 laje inteira | false = 1 por compartimento
    usinagem:    false, // usinagem do rebaixo de encaixe nas tampas
    custoUsin:   80,    // R$/ml custo usinagem
    vendaUsin:   150,   // R$/ml venda usinagem
    custoLaje:   120,   // R$/m² custo laje vedante
    vendaLaje:   200,   // R$/m² venda laje vedante
  }
};
...
function getTampasDims() {
  ...
  // Base das tampas = área ÚTIL do corpo (não a área externa)
  // Se há rodapé com avanço, as tampas ficam dentro da área útil
  var C_base = (d.AvRod > 0) ? d.CUtil : d.C;
  var L_base = (d.AvRod > 0) ? d.LUtil : d.L;

  // Área das tampas (descontando moldura/rebaixo em todos os lados)
  ...
  return {
    C_ext: C_base, L_ext: L_base,   // base utilizada (útil ou externa)
    C_extTotal: d.C, L_extTotal: d.L, // sempre externo total
    mol: mol,
...
function getDims() {
  ...
  // ── ÁREA ÚTIL — rodapé com avanço lateral ─────────────────────────
  ...
  return {
    C:    C_cm  / 100,   // comprimento EXTERNO em m
    L:    L_cm  / 100,   // largura EXTERNA em m
    ...
    // ── Rodapé e área útil ────────────────────────────────────────
    AvRod: AvRod_cm,              // avanço lateral rodapé (cm)
    CUtil: CUtil_cm / 100,        // comprimento ÚTIL do corpo (m)
    LUtil: LUtil_cm / 100,        // largura ÚTIL do corpo (m)
...
function calcularFull() {
  ...
  // ── Dimensões do CORPO para peças laterais ────────────────────────
  // Se há rodapé com avanço, os painéis do corpo ficam no perímetro ÚTIL
  // Rodapé externo é calculado separadamente como peça própria
  var C_corpo = (d.AvRod > 0) ? d.CUtil : d.C;
  var L_corpo = (d.AvRod > 0) ? d.LUtil : d.L;

  // Lateral esquerda: altura visual × largura ÚTIL
  ...
  // Rodapé: perímetro EXTERNO × altura — borda somente na frente
  if (SEL.pecas.rodape && d.Ab > 0) {
    var perimetro = 2 * (d.C + d.L);  // perímetro externo total
  ...
  // ── REBAIXO DE ENCAIXE (usinagem) + LAJE VEDANTE ─────────────────
  ...
  if (SEL.rebaixo && (SEL.rebaixo.usinagem || SEL.rebaixo.lajeVedante)) {
    ...
    // Perímetro interno de cada abertura (onde o rebaixo é usinado)
    ml_rebaixo  = tdReb.nTotal * 2 * (tdReb.C_cada + tdReb.L_cada);
    // Área da laje: inteira (1 peça cobrindo tudo) ou 1 por compartimento
    if (SEL.rebaixo.lajeInteira) {
      // 1 laje única cobrindo toda a área útil
      m2_laje_ved = d.CUtil * d.LUtil;
    } else {
      // 1 laje por compartimento (igual à área de cada abertura × nº tampas)
  ...
  // ─── 7. TOTAIS ───────────────────────────────────────────────────
  // Custo total: pedra + acabamento + civil + MO + extras + rebaixo/laje
  ...
  // Valor de venda: base aplica margem geral; rebaixo/laje usam venda própria
...
// ── ÁREA ÚTIL — atualiza os painéis de informação ─────────────────────
function atualizarAreaUtil() {
  var d = getDims();
  var avRod = d.AvRod || 0;

  // Box no card de tampas (④-B)
  var boxAmb = document.getElementById('divAreaUtil');
  if (boxAmb) {
    if (avRod > 0) {
      boxAmb.style.display = 'block';
      boxAmb.innerHTML =
        '<strong style="color:#4a80b5;">📐 Externo × Corpo Útil</strong><br>' +
        'Externo: <b>' + d.C_cm + ' × ' + d.L_cm + ' cm</b> &nbsp;|&nbsp; ' +
        'Rodapé: <b>' + avRod + 'cm/lado</b><br>' +
        'Corpo útil: <b style="color:#4a80b5;">' + d.CUtil_cm + ' × ' + d.LUtil_cm + ' cm</b> ' +
        '(' + (d.CUtil*d.LUtil).toFixed(3) + ' m²) — base de cálculo das peças';
    } else {
      boxAmb.style.display = 'none';
    }
  }

  // Info laje vedante
  var r;
  try { r = calcularFull(); } catch(e) { return; }
  var infoLaje = document.getElementById('lajeVedInfo');
  if (infoLaje) {
    if (r.m2_laje_ved > 0) {
      var lajeDesc = SEL.rebaixo.lajeInteira
        ? '1 laje inteira de ' + Math.round(d.CUtil_cm) + '×' + Math.round(d.LUtil_cm) + ' cm'
        : (td ? td.nTotal + ' lajes de ' + Math.round(td.C_cada*100) + '×' + Math.round(td.L_cada*100) + ' cm' : '');
      infoLaje.textContent = lajeDesc + ' · ' + r.m2_laje_ved.toFixed(3) + ' m² · R$ ' + r.venda_laje_ved.toFixed(2);
    } else if (SEL.rebaixo.lajeVedante) {
      var td3 = getTampasDims();
      infoLaje.textContent = SEL.rebaixo.lajeInteira
        ? '1 laje de ' + Math.round(d.CUtil_cm) + '×' + Math.round(d.LUtil_cm) + ' cm · ' + (d.CUtil*d.LUtil).toFixed(3) + ' m²'
        : td3.nTotal + ' lajes de ' + Math.round(td3.C_cada*100) + '×' + Math.round(td3.L_cada*100) + ' cm';
    }
  }

  // Info usinagem
  var infoUsin = document.getElementById('usinagemInfo');
  if (infoUsin && r.ml_rebaixo > 0) {
    infoUsin.textContent = r.ml_rebaixo.toFixed(2) + ' ml · ' +
      'R$ ' + (SEL.rebaixo.vendaUsin || 150) + '/ml = R$ ' + r.venda_rebaixo.toFixed(2);
  } else if (infoUsin) {
    var td2 = getTampasDims();
    var mlEst = (td2.nTotal * 2 * (td2.C_cada + td2.L_cada)).toFixed(2);
    infoUsin.textContent = mlEst + ' ml estimado';
  }
  ...
  if (badge) {
    ...
    badge.textContent = ativos.length ? ativos.join(' + ') : '';
  }
}
...
function renderResultado(o) {
  ...
  if (r.venda_rebaixo > 0 || r.venda_laje_ved > 0) {
    var vedTotal = r.venda_rebaixo + r.venda_laje_ved;
    var vedSub = [];
    if (r.venda_rebaixo > 0)  vedSub.push('Usinagem');
    if (r.venda_laje_ved > 0) vedSub.push('Laje vedante');
    gridCusto.push({ lbl:'Vedação + Usinagem', val:'R$ '+_fn(vedTotal), cl:'', sub:vedSub.join(' + ') });
  }
  ...
  // ── Rebaixo e Laje Vedante ─────────────────────────────────────────
  if (r.venda_rebaixo > 0 || r.venda_laje_ved > 0) {
    dh += '<div class="det-sec">🔧 Vedação e Usinagem</div>';
    if (r.venda_rebaixo > 0) {
      dh += '<div class="det-line">' +
        '<span class="det-k">Usinagem rebaixo de encaixe — <span style="color:var(--t4);font-size:.62rem">' +
        r.ml_rebaixo.toFixed(2) + ' ml × R$ ' + (SEL.rebaixo.vendaUsin||150) + '/ml</span></span>' +
        '<span class="det-v">R$ '+_TI_fm(r.venda_rebaixo)+'</span></div>';
    }
    if (r.venda_laje_ved > 0) {
      dh += '<div class="det-line">' +
        '<span class="det-k">Laje vedante de concreto — <span style="color:var(--t4);font-size:.62rem">' +
        r.m2_laje_ved.toFixed(3) + ' m² × R$ ' + (SEL.rebaixo.vendaLaje||200) + '/m²</span></span>' +
        '<span class="det-v">R$ '+_TI_fm(r.venda_laje_ved)+'</span></div>';
    }
  }
