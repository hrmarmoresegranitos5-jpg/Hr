...
var DEF_CFG = {
  ...
  civil: {
    cimento:        36,   // R$/saco 50kg CP-II  (traço concreto + argamassa + reboco)
    areia:          95,   // R$/m³ posto na obra  (concreto + argamassa de bloco + reboco)
    brita:         130,   // R$/m³ brita 3/4"    (só concreto estrutural)
    argamassa:      32,   // R$/saco 20kg AC-III  (assentamento de pedra natural)
    ferro38:        7.50, // R$/metro linear CA50 3/8" (10mm) — alicerce e alvenaria
    ferro516:       5.50, // R$/metro linear CA50 5/16" (8mm) — laje e distribuição
    malha:          28,   // R$/m² Q-92 soldada   (lajes)
    blocos:         3.50, // R$/unid bloco 14×19×39 cm (paredes ~12,5 blocos/m²)
    canaleta:       4.50, // R$/unid canaleta estrutural (cintas de amarração)
    trelica:        11,   // R$/metro linear TQ-25 (cintas + lajes)
    massa_plastica: 16    // R$/kg rejunte / massa plástica (bordas e juntas)
  },
...
function calcularFull() {
  ...
  var civil = {
    ...
    m_ferro38:         0,  // ferro CA50 3/8" 10mm (m linear) — alicerce + divisórias
    m_ferro516:        0,  // ferro CA50 5/16" 8mm (m linear) — laje + distribuição
    m2_reboco:         0,  // m² de parede rebocada (informativo)
    sacos_argam:       0,  // sacos AC-III 20kg — APENAS assentamento de pedra natural
  ...
  if (SEL.tipoServ === 'estrutura') {
    ...
    // Treliça: cintas (n_cintas × Perim) + lajes (N_lajes_est × C)
    civil.m_trelica = +(((n_cintas * Perim) + (N_lajes_est * d.C)) * 1.10).toFixed(1);
    ...
    //   Divisórias internas: nBarras × comprimento C; espaçamento 25cm nas divisórias
    //   Alvenaria perimetral: volta completa do perímetro a cada 30cm de altura (Avis/0.30 voltas)
    ...
    civil.custo =
      ...
      civil.m_ferro38        * cv.ferro38   +
      civil.m_ferro516       * cv.ferro516  +
...
function renderProducao() {
  ...
  var rows = '';
  var idx  = 1;
  // Fator de espessura (mesmo usado no cálculo principal de custo_pedra)
  var _espFatores = { 2:1.00, 3:1.35, 4:1.70, 5:2.10 };
  var _espFator   = _espFatores[r.d.E] || 1.35;
  r.pecasCalc.forEach(function(p) {
    var peso_peca = p.m2 * Esp_m * mat.peso;
    var custo_peca = p.m2 * mat.pr * _espFator; // c/ fator de espessura (igual ao cálculo principal)
  ...
  var _custoComPerda = totalM2 * (1 + r.perdaFinal/100) * mat.pr * _espFator;
  _gel('prodTotalPecas').textContent = (idx-1)+' peças · '+totalM2.toFixed(3)+' m² bruto (+'+r.perdaFinal+'% perda) · '+Math.round(totalPeso)+' kg';
  ...
  var foot = '<tr style="background:var(--bg3)">'
    ...
    +'<td style="padding:10px 10px;font-family:\'DM Mono\',monospace;font-weight:700;color:var(--gold2)">'+totalM2.toFixed(3)+' m² bruto</td>'
    +'<td style="padding:10px 10px;font-family:\'DM Mono\',monospace;font-weight:700;color:var(--tx)">'+Math.round(totalPeso)+' kg</td>'
    +'<td></td>'
    +'<td style="padding:10px 10px;font-family:\'DM Mono\',monospace;font-weight:700;color:var(--grn)" title="c/ '+r.perdaFinal+'% perda + fator esp.">R$ '+_TI_fm(_custoComPerda)+'</td>'
    +'</tr>';
  ...
  if (r.ts.id === 'estrutura') {
    ...
    // CORRECAO: horizontal tem apenas 1 laje de topo (não N); vertical tem N
    var _nLajEst = (d.disp === 'horizontal') ? 1 : Math.max(d.N, 1);
    var Vol_lajes   = d.C * d.L * Math.max(d.Hlaje - Esp_m, 0.06) * _nLajEst;
    ...
    hCivil += '<div class="det-line"><span class="det-k">Lajes de concreto ('+_nLajEst+' laje' +(_nLajEst>1?'s':'')+')</span><span class="det-v">'+Vol_lajes.toFixed(3)+' m³</span></div>';
    ...
    hCivil += '<div class="det-line"><span class="det-k">Ferro 3/8" (10mm)</span><span class="det-v">'+r.civil.m_ferro38+' m</span></div>';
    hCivil += '<div class="det-line"><span class="det-k">Ferro 5/16" (8mm)</span><span class="det-v">'+r.civil.m_ferro516+' m</span></div>';
    hCivil += '<div class="det-line"><span class="det-k">Treliça eletrossoldada TQ-25</span><span class="det-v">'+r.civil.m_trelica.toFixed(1)+' m</span></div>';
    ...
    hCivil += '<div class="det-line"><span class="det-k">Canaletas 14×19×39 — baldrame/cintas</span><span class="det-v">'+r.civil.unid_canaletas+' un.</span></div>';
    hCivil += '<div class="det-line"><span class="det-k">Argamassa de assentamento AC-III</span><span class="det-v">'+r.civil.sacos_argam+' sacos × 20kg</span></div>';
    hCivil += '<div class="det-line"><span class="det-k">Massa plástica (juntas entre pedras)</span><span class="det-v">'+r.civil.kg_massa_plastica.toFixed(2)+' kg</span></div>';
    ...
    hCivil += '<div class="callout" style="margin:0">Serviço <strong>'+r.ts.nm+'</strong> — estrutura civil não inclusa. Apenas argamassa de assentamento: <strong>'+r.civil.sacos_argam+' sacos AC-III</strong>.</div>';
  }
