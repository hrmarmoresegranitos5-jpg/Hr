...
function gerarPrintArea(o, r) {
  ...
  var heads = [['#','center'],['PEÇA / DESCRIÇÃO','left'],['DIMENSÕES (C×L)','center'],['ESP.','center'],['ÁREA m²','right'],['OBSERVAÇÃO','left']];
  heads.forEach(function(h) {
    p2 += '<th style="padding:7px 10px;text-align:'+h[1]+';font-size:7px;letter-spacing:1.5px;text-transform:uppercase;color:#C9A84C;font-weight:900;">'+h[0]+'</th>';
  });
  p2 += '</tr></thead><tbody>';
  r.pecasCalc.forEach(function(p, i) {
    ...
    if (nm.indexOf('tampa')>=0 && nm.indexOf('superior')>=0 && SEL.tampas.argolas) obs = (td.nTotal*2)+' argolas';
    if (nm.indexOf('lapide')>=0 || nm.indexOf('lápide')>=0) {
      if (nm.indexOf('frente')>=0) obs = 'Face visível';
      else if (nm.indexOf('traseira')>=0) obs = 'Colada na frente';
      else if (nm.indexOf('encontro')>=0) obs = 'Engrossamento';
    }
    p2 += '<tr>';
    p2 += '<td style="padding:7px 9px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:9px;color:#888;text-align:center;">'+(i+1)+'</td>';
    p2 += '<td style="padding:7px 9px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:11px;font-weight:700;color:#1a1a1a;">'+esc(p.nm)+'</td>';
    p2 += '<td style="padding:7px 9px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:11px;color:#555;text-align:center;">'+esc(p.dim||'—')+'</td>';
    p2 += '<td style="padding:7px 9px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:11px;color:#777;text-align:center;">'+d.E+' cm</td>';
    p2 += '<td style="padding:7px 9px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:11px;text-align:right;font-weight:700;color:#5a3800;">'+(p.m2>0?p.m2.toFixed(3)+' m²':'—')+'</td>';
    p2 += '<td style="padding:7px 9px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:9px;color:#888;">'+esc(obs)+'</td>';
    p2 += '</tr>';
  });
  p2 += '<tr style="background:#fdf5e0;"><td colspan="4" style="padding:7px 10px;font-size:9px;color:#888;font-style:italic;">+ Perda/Retalho ('+r.perdaFinal+'%) — cortes e transporte</td>';
  p2 += '<td style="padding:7px 10px;text-align:right;font-size:10px;color:#888;">+'+(r.m2_total-r.m2_bruto).toFixed(3)+' m²</td><td></td></tr>';
  p2 += '<tr style="background:#0f0c00;"><td colspan="4" style="padding:8px 10px;font-size:9px;font-weight:900;color:#C9A84C;letter-spacing:1px;">TOTAL COM PERDA</td>';
  p2 += '<td style="padding:8px 10px;text-align:right;font-size:13px;font-weight:900;color:#C9A84C;">'+r.m2_total.toFixed(3)+' m²</td><td></td></tr>';
  p2 += '</tbody></table></div>';

  if (extrasItems.length > 0) {
    p2 += sh('Acessórios e Serviços Adicionais');
    p2 += '<div style="border:1px solid #e8dfc4;border-radius:10px;overflow:hidden;margin-bottom:14px;">';
    extrasItems.forEach(function(ex, i) {
      var bg = i%2===0 ? '#fff' : '#faf6ef';
      p2 += '<div style="background:'+bg+';padding:7px 12px;border-bottom:1px solid #ede8dc;display:flex;justify-content:space-between;">';
      p2 += '<span style="font-size:11px;font-weight:700;color:#1a1a1a;">'+ex.i+' '+esc(ex.l)+'</span>';
      p2 += '<span style="font-size:11px;color:#555;">'+esc(ex.v)+'</span></div>';
    });
    p2 += '</div>';
  }

  // ─ Custos (interno) ─
  p2 += sh('Composição de Custos — Uso Interno');
  p2 += '<div style="border:1px solid #e8e0d0;border-radius:10px;overflow:hidden;margin-bottom:14px;">';
  var costItems = [
    { l:'🪨 Pedra — '+r.m2_total.toFixed(3)+' m² × R$'+mat.pr+'/m²', v:r.custo_pedra },
    { l:'✨ Acabamento bordas', v:r.custo_acabamento },
    { l:'🏗️ Material Civil ('+r.ts.nm+')', v:r.civil.custo },
    { l:'🔨 Mão de Obra', v:r.custo_mob },
  ];
  if (r.custo_extras > 0) costItems.push({ l:'✨ Extras (acessórios)', v:r.custo_extras });
  if (r.custo_rebaixo > 0) costItems.push({ l:'🔧 Usinagem rebaixo — '+r.ml_rebaixo.toFixed(2)+' ml', v:r.custo_rebaixo });
  if (r.custo_laje_ved > 0) costItems.push({ l:'🧱 Laje vedante — '+r.m2_laje_ved.toFixed(3)+' m²', v:r.custo_laje_ved });
  costItems.forEach(function(it, i) {
    if (!it.v) return;
    var bg = i%2===0 ? '#fff' : '#faf6ef';
    p2 += '<div style="background:'+bg+';padding:7px 12px;border-bottom:1px solid #ede8dc;display:flex;justify-content:space-between;">';
    p2 += '<span style="font-size:11px;color:#333;">'+esc(it.l)+'</span>';
    p2 += '<span style="font-size:11px;font-weight:700;color:#1a1a1a;">'+fv(it.v)+'</span></div>';
  });
  p2 += '<div style="background:#0f0c00;padding:9px 12px;display:flex;justify-content:space-between;">';
  p2 += '<span style="font-size:9px;font-weight:900;color:#C9A84C;letter-spacing:1px;">CUSTO TOTAL</span>';
  p2 += '<span style="font-size:13px;font-weight:900;color:#C9A84C;">'+fv(r.custo_total)+'</span></div>';
  p2 += '</div>';

  p2 += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">';
  p2 += '<div style="background:#fdfaf3;border:1px solid #e8dfc4;border-radius:8px;padding:10px 12px;">';
  p2 += '<div style="font-size:6.5px;letter-spacing:1.5px;color:#9a7840;font-weight:900;text-transform:uppercase;margin-bottom:3px;">VENDA TOTAL À VISTA</div>';
  p2 += '<div style="font-size:15px;font-weight:900;color:#5a3a00;">'+fv(vista)+'</div></div>';
  p2 += '<div style="background:#f0faf3;border:1px solid #c0e8d0;border-radius:8px;padding:10px 12px;">';
  p2 += '<div style="font-size:6.5px;letter-spacing:1.5px;color:#408060;font-weight:900;text-transform:uppercase;margin-bottom:3px;">MARGEM BRUTA ('+CFG.margem+'%)</div>';
  p2 += '<div style="font-size:15px;font-weight:900;color:#2a6040;">'+fv(r.margem_reais)+'</div></div>';
  p2 += '</div>';
  ...
  p2 += '<div style="font-size:8px;color:rgba(255,255,255,.15);">Documento interno — não enviar ao cliente</div></div>';
