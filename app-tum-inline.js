...
var pendOrc = null;
var delIdx = -1;
var _tumFotoOrc = null;  // foto base64 do modelo (aparece no PDF)
...
function getTampasDims() {
  ...
  return {
    C_ext: C_base, L_ext: L_base, C_extTotal: d.C, L_extTotal: d.L,
    mol: mol,
...
function gerarPrintArea(o, r) {
  ...
  var mat = r.mat;
  var acab = r.acab;
  var pdfCount = parseInt(localStorage.getItem('hr_pdf_count_tum')||'0', 10);
  var orcNum = o.num || ('ORC-'+String(pdfCount).padStart(4,'0'));
  localStorage.setItem('hr_pdf_count_tum', pdfCount + 1);

  function fv(v) { return 'R$\u00a0' + _TI_fm(v); }
  function sh(t) {
    return '<div style="font-size:7px;letter-spacing:3px;text-transform:uppercase;color:#7a4e00;font-weight:900;margin:0 0 8px;padding:0 0 5px;border-bottom:2px solid #C9A84C;">' + t + '</div>';
  }
  ...
  if (SEL.pecas.tampa) {
    var tdesc = td.nTotal + ' tampa' + (td.nTotal>1?'s':'') + ' — '
      + Math.round(td.C_cada*100) + '×' + Math.round(td.L_cada*100) + ' cm'
      + ', esp. ' + (SEL.tampas.espTampa||3) + ' cm';
    if (SEL.tampas.argolas) tdesc += ' · ' + (td.nTotal*2) + ' argolas de içamento';
    extrasItems.push({ i:'🪨', l:'Tampas Superiores ('+td.nTotal+'×)', v:tdesc });
  }
  if (SEL.pecas.lapide) {
    var lDesc = d.LapW_cm + '×' + d.LapH_cm + ' cm';
    if (engCm > 0) lDesc += ' · Engrossada ' + engCm + 'cm (dupla)';
    extrasItems.push({ i:'📜', l:'Lápide', v:lDesc });
  }
  if (SEL.opts.foto_porc && r.nFotos > 0)
    extrasItems.push({ i:'📷', l:'Foto em Porcelana', v:r.nFotos + ' unid.' });
  if (SEL.opts.cruzGranito && r.nCruz > 0)
    extrasItems.push({ i:'✝', l:'Cruz em Granito', v:r.nCruz + ' unid.' });
  if (SEL.opts.jarro && r.nJarros > 0)
    extrasItems.push({ i:'🏺', l:'Jarros Decorativos', v:r.nJarros + ' par(es)' });
  if (SEL.opts.lapide45)
    extrasItems.push({ i:'📐', l:'Lápide Chanfrada 45°', v:'Bisotado nas 3 arestas' });
  if (SEL.rebaixo && SEL.rebaixo.lajeVedante && r.m2_laje_ved > 0) {
    var lj = SEL.rebaixo.lajeInteira
      ? '1 laje inteira — ' + Math.round(d.CUtil_cm) + '×' + Math.round(d.LUtil_cm) + ' cm (' + r.m2_laje_ved.toFixed(3) + ' m²)'
      : td.nTotal + ' lajes — ' + Math.round(td.C_cada*100) + '×' + Math.round(td.L_cada*100) + ' cm';
    extrasItems.push({ i:'🧱', l:'Laje Vedante de Concreto', v:lj });
  }
  if (SEL.rebaixo && SEL.rebaixo.usinagem && r.ml_rebaixo > 0)
    extrasItems.push({ i:'🔧', l:'Usinagem Rebaixo', v:r.ml_rebaixo.toFixed(2) + ' ml' });

  // ══ PÁGINA 1 — ORÇAMENTO AO CLIENTE ══════════════════════════════════════
  var p1 = '';
  p1 += '<div style="height:5px;background:linear-gradient(90deg,#3a2500,#C9A84C,#E8C96A,#C9A84C,#3a2500);"></div>';
  // Header
  p1 += '<div style="background:#0f0c00;padding:20px 28px 16px;display:flex;justify-content:space-between;align-items:flex-start;">';
  p1 += '<div><div style="font-size:22px;font-weight:900;color:#C9A84C;letter-spacing:-.3px;">'+esc(emp.nome||'HR Mármores e Granitos')+'</div>';
  p1 += '<div style="font-size:7px;letter-spacing:3px;color:rgba(201,168,76,.4);margin-top:3px;">MÁRMORE · GRANITO · QUARTZITO</div>';
  p1 += '<div style="font-size:9px;color:rgba(255,255,255,.2);font-style:italic;margin-top:2px;">Qualidade, Precisão e Acabamento Profissional</div></div>';
  ...
  if (emp.ig)     p1 += '<div style="font-size:9px;color:rgba(255,255,255,.3);">'+esc(emp.ig)+'</div>';
  ...
  p1 += '<div style="display:flex;align-items:center;gap:10px;">';
  p1 += '<div style="background:#0f0c00;color:#C9A84C;font-size:7px;font-weight:900;padding:5px 14px;border-radius:20px;letter-spacing:2px;border:1px solid rgba(201,168,76,.4);">⚱️ ORÇAMENTO</div>';
  p1 += '<div style="background:#C9A84C;color:#000;font-size:8px;font-weight:900;padding:4px 10px;border-radius:5px;letter-spacing:1px;">'+esc(orcNum)+'</div>';
  p1 += '</div>';
  p1 += '<div style="font-size:9.5px;color:#666;"><strong style="color:#5a3800;">EMISSÃO:</strong> '+esc(o.date||'')+'&nbsp;&nbsp;|&nbsp;&nbsp;Validade: 7 dias</div>';
  p1 += '</div>';
  // Body
  ...
  if (Array.isArray(o.fal)) {
    o.fal.forEach(function(f) {
      if (!f.nome) return;
      p1 += '<div style="font-size:10px;color:#777;margin-top:3px;">⚰️ <strong>'+esc(f.nome)+'</strong>';
      if (f.nasc||f.obit) p1 += ' ('+esc(f.nasc||'?')+' – '+esc(f.obit||'?')+')';
      p1 += '</div>';
      if (f.frase) p1 += '<div style="font-size:9px;color:#999;font-style:italic;">&#8220;'+esc(f.frase)+'&#8221;</div>';
    });
  }
  ...
  p1 += '</div>';
  if (_tumFotoOrc) {
    p1 += '<div style="width:200px;flex-shrink:0;border-radius:10px;overflow:hidden;border:2px solid #C9A84C;">';
    p1 += '<img src="'+_tumFotoOrc+'" style="width:100%;height:100%;object-fit:cover;display:block;">';
    p1 += '</div>';
  } else {
    p1 += '<div style="background:#0f0c00;border:1px solid rgba(201,168,76,.4);border-radius:10px;padding:13px 16px;text-align:center;display:flex;flex-direction:column;justify-content:center;min-width:130px;">';
    p1 += '<div style="font-size:28px;margin-bottom:6px;">⚰️</div>';
    p1 += '<div style="font-size:14px;font-weight:900;color:#C9A84C;">Túmulo</div>';
    p1 += '<div style="font-size:9px;color:rgba(255,255,255,.25);margin-top:5px;">'+d.C_cm+' × '+d.L_cm+' × '+(r.A*100).toFixed(0)+' cm</div>';
    p1 += '</div>';
  }
  p1 += '</div>';
  ...
  var specs = [
    {l:'DIMENSÃO EXTERNA', v:d.C_cm+' × '+d.L_cm+' cm'},
    {l:'ÁREA ÚTIL DO CORPO', v:d.CUtil_cm+' × '+d.LUtil_cm+' cm'},
    {l:'ALTURA TOTAL', v:(r.A*100).toFixed(0)+' cm'},
    {l:'COMPARTIMENTOS', v:d.N+(d.N===0?' (simples)':(d.disp==='horizontal'?' lado a lado':' vertical'))},
    {l:'TIPO DE SERVIÇO', v:o.tipoServNm||'—'},
    {l:'ESPESSURA DA PEDRA', v:d.E+' cm'},
    {l:'ÁREA TOTAL DE PEDRA', v:r.m2_total.toFixed(3)+' m²'},
    {l:'PESO APROX.', v:Math.round(r.peso_total)+' kg'},
  ];
  p1 += '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-bottom:16px;">';
  specs.forEach(function(sp) {
    p1 += '<div style="background:#fdfaf3;border:1px solid #e8dfc4;border-radius:8px;padding:9px 11px;">';
    p1 += '<div style="font-size:6px;letter-spacing:1.5px;text-transform:uppercase;color:#9a7840;font-weight:900;margin-bottom:3px;">'+sp.l+'</div>';
    p1 += '<div style="font-size:12.5px;font-weight:700;color:#1a1a1a;">'+sp.v+'</div></div>';
  });
  p1 += '</div>';
  ...
  p1 += '<div><div style="font-size:7px;letter-spacing:2px;color:rgba(201,168,76,.5);font-weight:900;margin-bottom:4px;">MATERIAL SELECIONADO</div>';
  p1 += '<div style="font-size:17px;font-weight:900;color:#C9A84C;">'+esc(mat.nm||o.matNm||'—')+'</div>';
  p1 += '<div style="font-size:9px;color:rgba(255,255,255,.4);margin-top:2px;">'+(mat.cat||'')+(mat.cat&&mat.fin?' · ':'')+( mat.fin||'')+(mat.pr?' · R$ '+mat.pr+'/m²':'')+'</div></div>';
  p1 += '<div style="text-align:right;"><div style="font-size:7px;letter-spacing:1.5px;color:rgba(255,255,255,.3);font-weight:900;margin-bottom:3px;">ACABAMENTO</div>';
  p1 += '<div style="font-size:15px;font-weight:700;color:rgba(255,255,255,.8);">'+esc(acab.nm||o.acabNm||'—')+'</div></div></div>';

  // ─ Acessórios ─
  if (extrasItems.length > 0) {
    p1 += sh('Componentes e Acessórios Incluídos');
    p1 += '<div style="border:1px solid #e8dfc4;border-radius:10px;overflow:hidden;margin-bottom:16px;">';
    extrasItems.forEach(function(ex, i) {
      var bg = i%2===0 ? '#fff' : '#fdfaf3';
      p1 += '<div style="background:'+bg+';padding:8px 13px;border-bottom:1px solid #ede8dc;display:flex;justify-content:space-between;align-items:center;">';
      p1 += '<span style="font-size:11px;font-weight:700;color:#1a1a1a;">'+ex.i+' '+esc(ex.l)+'</span>';
      p1 += '<span style="font-size:11px;color:#555;">'+esc(ex.v)+'</span></div>';
    });
    p1 += '</div>';
  }
  ...
  var vista = r.valor_vista;
  var parcTotal = vista * (1 + CFG.juros/100);
  var pMensal = parcTotal / CFG.parcMax;
  var economia = parcTotal - vista;
  var entrada = vista * 0.5;

  p1 += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">';
  p1 += '<div style="background:#fdfaf3;border:1px solid #e8dfc4;border-radius:10px;padding:13px 15px;">';
  p1 += '<div style="font-size:6.5px;letter-spacing:1.5px;color:#9a7840;font-weight:900;text-transform:uppercase;margin-bottom:5px;">PARCELADO</div>';
  p1 += '<div style="font-size:18px;font-weight:900;color:#5a3a00;">'+fv(parcTotal)+'</div>';
  p1 += '<div style="font-size:9.5px;color:#888;margin-top:2px;">até '+CFG.parcMax+'× de '+fv(pMensal)+'</div></div>';
  p1 += '<div style="background:#0f0c00;border:2px solid #C9A84C;border-radius:10px;padding:13px 15px;position:relative;">';
  p1 += '<div style="position:absolute;top:8px;right:10px;background:#C9A84C;color:#000;font-size:6.5px;font-weight:900;padding:2px 8px;border-radius:3px;">MELHOR OPÇÃO</div>';
  p1 += '<div style="font-size:6.5px;letter-spacing:1.5px;color:rgba(201,168,76,.5);font-weight:900;text-transform:uppercase;margin-bottom:5px;">A VISTA</div>';
  p1 += '<div style="font-size:21px;font-weight:900;color:#C9A84C;">'+fv(vista)+'</div>';
  p1 += '<div style="font-size:9px;color:rgba(255,255,255,.3);">Valor final sem juros</div>';
  p1 += '<div style="font-size:9px;color:#6aaa80;margin-top:2px;">▼ Economia de '+fv(economia)+'</div></div>';
  p1 += '</div>';
  p1 += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">';
  p1 += '<div style="background:#fdfaf3;border:1px solid #e8dfc4;border-radius:10px;padding:11px 15px;">';
  p1 += '<div style="font-size:6px;letter-spacing:1.5px;color:#9a7840;font-weight:900;text-transform:uppercase;margin-bottom:3px;">ENTRADA — 50%</div>';
  p1 += '<div style="font-size:17px;font-weight:900;color:#5a3a00;">'+fv(entrada)+'</div>';
  p1 += '<div style="font-size:9px;color:#888;margin-top:2px;">Na assinatura / medição</div></div>';
  p1 += '<div style="background:#fdfaf3;border:1px solid #e8dfc4;border-radius:10px;padding:11px 15px;">';
  p1 += '<div style="font-size:6px;letter-spacing:1.5px;color:#9a7840;font-weight:900;text-transform:uppercase;margin-bottom:3px;">NA ENTREGA — 50%</div>';
  p1 += '<div style="font-size:17px;font-weight:900;color:#5a3a00;">'+fv(entrada)+'</div>';
  p1 += '<div style="font-size:9px;color:#888;margin-top:2px;">Na entrega / instalação</div></div>';
  p1 += '</div>';
  p1 += '<div style="background:#0f0c00;border-radius:8px;padding:8px 14px;display:flex;justify-content:space-between;margin-bottom:14px;">';
  p1 += '<span style="font-size:9px;color:rgba(255,255,255,.35);">⏱ PRAZO ESTIMADO</span>';
  p1 += '<span style="font-size:12px;font-weight:700;color:#C9A84C;">'+r.prazo_total+' dias úteis</span></div>';
  if (o.obs) {
    p1 += '<div style="background:#fffbf0;border-left:3px solid #C9A84C;padding:9px 13px;margin-bottom:12px;font-size:11px;color:#555;border-radius:0 8px 8px 0;">';
    p1 += '<strong style="color:#7a4e00;">Obs:</strong> '+esc(o.obs)+'</div>';
  }
  p1 += '</div>';
  p1 += '<div style="background:#0f0c00;padding:9px 28px;display:flex;justify-content:space-between;border-top:1px solid rgba(201,168,76,.15);">';
  p1 += '<div style="font-size:9px;color:rgba(201,168,76,.55);">'+esc(emp.nome||'')+' · '+esc(emp.tel||'')+'</div>';
  p1 += '<div style="font-size:8px;color:rgba(255,255,255,.15);">CNPJ: '+esc(emp.cnpj||'—')+'</div></div>';
  ...
  p2 += '<div style="background:#0f0c00;padding:11px 28px;display:flex;justify-content:space-between;align-items:center;">';
  p2 += '<div style="font-size:14px;font-weight:900;color:#C9A84C;">'+esc(emp.nome||'')+'</div>';
  p2 += '<div style="display:flex;gap:8px;align-items:center;">';
  p2 += '<div style="background:rgba(201,168,76,.1);border:1px solid rgba(201,168,76,.3);color:#C9A84C;font-size:7px;font-weight:900;padding:4px 10px;border-radius:4px;letter-spacing:1.5px;">DETALHAMENTO TÉCNICO</div>';
  p2 += '<div style="background:#C9A84C;color:#000;font-size:8px;font-weight:900;padding:3px 8px;border-radius:4px;">'+esc(orcNum)+'</div>';
  p2 += '</div></div>';
  p2 += '<div style="background:#faf6ee;border-bottom:2px solid rgba(201,168,76,.25);padding:7px 28px;">';
  p2 += '<span style="font-size:9px;color:#888;">Cliente: <strong style="color:#333;">'+esc(o.cli||'—')+'</strong>';
  p2 += ' &nbsp;·&nbsp; Material: <strong style="color:#333;">'+esc(mat.nm||o.matNm||'—')+'</strong>';
  p2 += ' &nbsp;·&nbsp; '+esc(o.date||'')+'</span></div>';
  p2 += '<div style="padding:14px 28px;">';

  // ─ Peças em pedra com peso ─────────────────────────────────────────────
  p2 += sh('🪨 Lista de Peças em Pedra — Medidas Exatas');
  var densKgM2 = 2700 * (d.E / 100);  // kg/m² para a espessura configurada
  p2 += '<div style="border:1px solid #e8e0d0;border-radius:10px;overflow:hidden;margin-bottom:14px;">';
  p2 += '<table style="width:100%;border-collapse:collapse;">';
  p2 += '<thead><tr style="background:#0f0c00;">';
  var heads2 = [['#','center'],['PEÇA','left'],['COMP × LARG (cm)','center'],['ESP.','center'],['ÁREA m²','right'],['PESO kg','right'],['OBS','left']];
  heads2.forEach(function(h) {
    p2 += '<th style="padding:7px 9px;text-align:'+h[1]+';font-size:6.5px;letter-spacing:1.5px;text-transform:uppercase;color:#C9A84C;font-weight:900;">'+h[0]+'</th>';
  });
  p2 += '</tr></thead><tbody>';
  var totalPeso = 0;
  r.pecasCalc.forEach(function(p, i) {
    var bg = i%2===0 ? '#fff' : '#faf6ef';
    var peso = +(p.m2 * densKgM2).toFixed(1);
    totalPeso += peso;
    var obs = '';
    var nm = (p.nm||'').toLowerCase();
    if ((nm.indexOf('tampa')>=0) && SEL.tampas.argolas) obs = (td.nTotal*2)+' argolas';
    if (nm.indexOf('lapide')>=0||nm.indexOf('lápide')>=0) {
      if (nm.indexOf('frente')>=0) obs = 'Face visível — polida';
      else if (nm.indexOf('traseira')>=0||nm.indexOf('encontro')>=0) obs = 'Engrossamento';
    }
    if (nm.indexOf('rodapé')>=0||nm.indexOf('rodape')>=0) obs = 'Perímetro externo';
    if (nm.indexOf('divisória')>=0||nm.indexOf('divisoria')>=0) obs = 'Interna — sem acabamento';
    // Parse dim to extract C×L
    var dimClean = esc(p.dim||'—');
    p2 += '<tr>';
    p2 += '<td style="padding:7px 8px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:9px;color:#888;text-align:center;">'+(i+1)+'</td>';
    p2 += '<td style="padding:7px 8px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:10.5px;font-weight:700;color:#1a1a1a;">'+esc(p.nm)+'</td>';
    p2 += '<td style="padding:7px 8px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:10px;color:#555;text-align:center;">'+dimClean+'</td>';
    ...
    p2 += '<td style="padding:7px 8px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:10px;text-align:right;font-weight:700;color:#5a3800;">'+(p.m2>0?p.m2.toFixed(3)+' m²':'—')+'</td>';
    p2 += '<td style="padding:7px 8px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:10px;text-align:right;color:#444;">'+peso+' kg</td>';
    p2 += '<td style="padding:7px 8px;background:'+bg+';border-bottom:1px solid #ede8dc;font-size:8.5px;color:#888;">'+esc(obs)+'</td>';
    p2 += '</tr>';
  });
  p2 += '<tr style="background:#fdf5e0;"><td colspan="4" style="padding:6px 9px;font-size:9px;color:#888;font-style:italic;">+ Perda/Retalho ('+r.perdaFinal+'%) — cortes e transporte</td>';
  p2 += '<td style="padding:6px 9px;text-align:right;font-size:10px;color:#888;">+'+(r.m2_total-r.m2_bruto).toFixed(3)+' m²</td><td colspan="2"></td></tr>';
  p2 += '<tr style="background:#0f0c00;">';
  p2 += '<td colspan="4" style="padding:8px 9px;font-size:9px;font-weight:900;color:#C9A84C;letter-spacing:1px;">TOTAL (COM PERDA)</td>';
  p2 += '<td style="padding:8px 9px;text-align:right;font-size:12px;font-weight:900;color:#C9A84C;">'+r.m2_total.toFixed(3)+' m²</td>';
  p2 += '<td style="padding:8px 9px;text-align:right;font-size:11px;font-weight:700;color:rgba(201,168,76,.7);">'+Math.round(r.peso_total)+' kg</td>';
  p2 += '<td></td></tr>';
  p2 += '</tbody></table></div>';
  ...
  if (d.N > 0) {
    ...
    if (SEL.rebaixo.lajeVedante) {
      specs2.push({ l:'Laje vedante de concreto', v: SEL.rebaixo.lajeInteira ? '1 laje inteira '+Math.round(d.CUtil_cm)+'×'+Math.round(d.LUtil_cm)+' cm' : td.nTotal+' lajes — '+Math.round(td.C_cada*100)+'×'+Math.round(td.L_cada*100)+' cm' });
    }
    specs2.forEach(function(sp, i) {
      var bg = i%2===0 ? '#fff' : '#fdfaf3';
      p2 += '<div style="background:'+bg+';padding:8px 13px;border-bottom:1px solid #ede8dc;display:flex;justify-content:space-between;">';
      p2 += '<span style="font-size:10.5px;color:#555;">'+sp.l+'</span>';
      p2 += '<span style="font-size:10.5px;font-weight:700;color:#1a1a1a;">'+sp.v+'</span></div>';
    });
    p2 += '</div>';
  }

  // ─ Estrutura civil (quantitativos) ─────────────────────────────────────
  if (r.civil && (r.civil.sacos_cimento > 0 || r.civil.unid_blocos > 0)) {
    p2 += sh('🏗️ Quantitativo Civil — Estrutura Completa');
    var cv = r.civil;
    var civRows = [];
    if (cv.sacos_cimento > 0) civRows.push({ l:'Cimento CP-II (sacos 50 kg)', v:cv.sacos_cimento+' sacos' });
    if (cv.m3_areia > 0)     civRows.push({ l:'Areia lavada', v:cv.m3_areia.toFixed(2)+' m³' });
    if (cv.m3_brita > 0)     civRows.push({ l:'Brita 3/4"', v:cv.m3_brita.toFixed(2)+' m³' });
    if (cv.sacos_argam > 0)  civRows.push({ l:'Argamassa AC-II (sacos 20 kg)', v:cv.sacos_argam+' sacos' });
    if (cv.barras_f38 > 0)   civRows.push({ l:'Ferro 3/8" (barras 12 m)', v:cv.barras_f38+' barras' });
    if (cv.barras_f516 > 0)  civRows.push({ l:'Ferro 5/16" (barras 12 m)', v:cv.barras_f516+' barras' });
    if (cv.m2_malha > 0)     civRows.push({ l:'Malha soldada Q-92', v:cv.m2_malha.toFixed(2)+' m²' });
    if (cv.unid_blocos > 0)  civRows.push({ l:'Blocos de concreto 14×19×39 cm', v:cv.unid_blocos+' unidades' });
    p2 += '<div style="border:1px solid #e8dfc4;border-radius:10px;overflow:hidden;margin-bottom:14px;">';
    civRows.forEach(function(it, i) {
      var bg = i%2===0 ? '#fff' : '#fdfaf3';
      p2 += '<div style="background:'+bg+';padding:8px 13px;border-bottom:1px solid #ede8dc;display:flex;justify-content:space-between;">';
      p2 += '<span style="font-size:10.5px;color:#333;">'+it.l+'</span>';
      p2 += '<span style="font-size:10.5px;font-weight:700;color:#1a1a1a;">'+it.v+'</span></div>';
    });
    p2 += '</div>';
  }

  // ─ Mão de obra ─────────────────────────────────────────────────────────
  p2 += sh('🔨 Mão de Obra Estimada');
  var mobRows = [];
  if (r.dias_ped   > 0) mobRows.push({ l:'Pedreiro',    v:r.dias_ped   +' dia(s)', s:'Fundação e alvenaria' });
  if (r.dias_ajud  > 0) mobRows.push({ l:'Ajudante',    v:r.dias_ajud  +' dia(s)', s:'Suporte geral' });
  if (r.dias_inst  > 0) mobRows.push({ l:'Instalação de pedra', v:r.dias_inst+' dia(s)', s:'Assentamento e polimento' });
  if (r.dias_mont  > 0) mobRows.push({ l:'Montagem final', v:r.dias_mont+' dia(s)', s:'Colocação tampas e acabamentos' });
  if (r.custo_remocao > 0) mobRows.push({ l:'Remoção de túmulo antigo', v:'Incluso', s:'' });
  if (r.frete      > 0) mobRows.push({ l:'Frete / Transporte', v:'Incluso', s:'' });
  p2 += '<div style="border:1px solid #e8dfc4;border-radius:10px;overflow:hidden;margin-bottom:14px;">';
  if (mobRows.length === 0) {
    p2 += '<div style="padding:12px 13px;font-size:10px;color:#888;background:#fff;">Conforme tipo de serviço selecionado</div>';
  }
  mobRows.forEach(function(it, i) {
    var bg = i%2===0 ? '#fff' : '#fdfaf3';
    p2 += '<div style="background:'+bg+';padding:8px 13px;border-bottom:1px solid #ede8dc;display:flex;justify-content:space-between;align-items:center;">';
    p2 += '<div><div style="font-size:10.5px;color:#333;font-weight:600;">'+it.l+'</div>';
    if (it.s) p2 += '<div style="font-size:8.5px;color:#888;margin-top:1px;">'+it.s+'</div>';
    p2 += '</div><span style="font-size:10.5px;font-weight:700;color:#1a1a1a;">'+it.v+'</span></div>';
  });
  var prazoLabel = r.prazo_total > 0 ? r.prazo_total+' dias úteis' : 'A definir';
  p2 += '<div style="background:#f7f2e8;padding:9px 13px;display:flex;justify-content:space-between;font-size:10.5px;font-weight:700;">';
  p2 += '<span style="color:#5a3800;">⏱ Prazo Total Estimado</span><span style="color:#1a1a1a;">'+prazoLabel+'</span></div>';
  p2 += '</div>';

  // ─ Configurações técnicas utilizadas ───────────────────────────────────
  p2 += sh('⚙️ Parâmetros do Projeto');
  var params = [
    { l:'Tipo de serviço', v:esc(r.ts?r.ts.nm:o.tipoServNm||'—') },
    { l:'Material da pedra', v:esc(mat.nm||o.matNm||'—') + (mat.pr?' — R$ '+mat.pr+'/m²':'') },
    { l:'Acabamento das bordas', v:esc(acab.nm||o.acabNm||'—') },
    { l:'Espessura da pedra', v:d.E+' cm' },
    { l:'Densidade estimada (granito/mármore)', v:'2.700 kg/m³' },
    { l:'Fator de perda/retalho', v:r.perdaFinal+'%' },
    { l:'Margem comercial', v:(CFG.margem||0)+'%' },
  ];
  if (extrasItems.length > 0) {
    params.push({ l:'Acessórios incluídos', v:extrasItems.map(function(e){return e.i+' '+e.l;}).join(', ') });
  }
  p2 += '<div style="border:1px solid #e8dfc4;border-radius:10px;overflow:hidden;margin-bottom:14px;">';
  params.forEach(function(it, i) {
    var bg = i%2===0 ? '#fff' : '#fdfaf3';
    p2 += '<div style="background:'+bg+';padding:7px 13px;border-bottom:1px solid #ede8dc;display:flex;justify-content:space-between;">';
    p2 += '<span style="font-size:10px;color:#555;">'+it.l+'</span>';
    p2 += '<span style="font-size:10px;font-weight:600;color:#1a1a1a;">'+it.v+'</span></div>';
  });
  p2 += '</div>';

  p2 += '</div>';
  p2 += '<div style="background:#0f0c00;padding:8px 28px;display:flex;justify-content:space-between;border-top:1px solid rgba(201,168,76,.15);">';
  p2 += '<div style="font-size:8px;color:rgba(201,168,76,.45);">'+esc(emp.nome||'')+' · '+esc(emp.tel||'')+' · '+esc(emp.end||'')+'</div>';
  p2 += '<div style="font-size:8px;color:rgba(255,255,255,.15);">Documento interno — revisão técnica</div></div>';

  _gel('pTitle').textContent = '';
  _gel('pMeta').textContent = '';
  _gel('pBody').innerHTML =
    '<div style="font-family:Arial,Helvetica,sans-serif;background:#fff;color:#1a1a1a;max-width:740px;margin:0 auto;">' + p1 + '</div>' +
    '<div style="font-family:Arial,Helvetica,sans-serif;background:#fff;color:#1a1a1a;max-width:740px;margin:0 auto;">' + p2 + '</div>';
...
// ── Tampa posicao presets ─────────────────────────────────────────────────
function buildPosicaoPresets() {
  var el = document.getElementById('posicaoPresets');
  if (!el) return;
  var opts = [
    { id:'superior', label:'🪨 Superior (em cima)',  dica:'Tampas deitadas horizontais — abertura de cima' },
  ...
  el.innerHTML = opts.map(function(o) {
    var on = o.id === cur;
    return '<button class="preset'+(on?' on':'')+'" onclick="_setPosicao(\''+o.id+'\')">'+o.label+'</button>';
  }).join('');
  var dica = document.getElementById('tampaPosDica');
  if (dica) {
    var found = opts.find(function(o){ return o.id === cur; });
    if (found) dica.textContent = found.dica;
  }
}
...
// ── Baixar PDF como arquivo HTML diretamente ─────────────────────────────
// O arquivo abre direto no Chrome → usuário usa menu "Imprimir" → "Salvar como PDF"
// Depois pode compartilhar do gerenciador de arquivos (WhatsApp, Drive, etc.)
function baixarPDF() {
  if (!pendOrc) { toast('Gere um orçamento primeiro', true); return; }
  var pBody = document.getElementById('pBody');
  if (!pBody || !pBody.innerHTML.trim()) {
    toast('Recalcule o orçamento antes de baixar', true); return;
  }
  var emp = CFG && CFG.emp ? CFG.emp : {};
  var html = '<!DOCTYPE html><html lang="pt-BR"><head>'
    + '<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
    + '<title>Orçamento — ' + (emp.nome||'HR Mármores') + '</title>'
    + '<style>@page{size:A4;margin:0}body{margin:0;padding:0;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}'
    + '#pdfPage1{page-break-after:always}#pdfPage2{page-break-after:auto}</style>'
    + '</head><body>' + pBody.innerHTML + '</body></html>';

  var nomeCliente = (pendOrc.cli || 'orcamento').replace(/[^a-zA-Z0-9áéíóúãõçÁÉÍÓÚÃÕÇ\s]/g, '').replace(/\s+/g, '_');
  var nomeArq = 'Orcamento_' + nomeCliente + '.html';
  ...
  try {
    ...
    // Fallback se blob não funcionar
...
// Inject photo upload area into the client card after mount
(function() {
  var _origMount = window.tumInlineMount;
  window.tumInlineMount = function(ambId) {
    _origMount(ambId);
    // Add photo upload if not already present
    setTimeout(function() {
      var loteEl = document.getElementById('iLote');
      if (loteEl && !document.getElementById('tumFotoArea')) {
        var div = document.createElement('div');
        div.className = 'f';
        div.style.cssText = 'grid-column:1/-1';
        div.innerHTML = '<label style="font-size:.62rem;color:var(--t3);letter-spacing:.05em;text-transform:uppercase;font-weight:500;">📷 Foto do modelo (aparece no PDF)</label>'
          + '<div id="tumFotoArea" onclick="document.getElementById(\'tumFotoInput\').click()" '
          + 'style="border:2px dashed var(--bd2);border-radius:10px;padding:10px;text-align:center;cursor:pointer;min-height:44px;display:flex;align-items:center;justify-content:center;margin-top:4px;transition:border-color .2s;">'
          + '<input type="file" id="tumFotoInput" accept="image/*" style="display:none" onchange="carregarFotoOrc(this)">'
          + '<div id="tumFotoPreview" style="font-size:.7rem;color:var(--t4)">📷 Adicionar foto do túmulo ao PDF</div>'
          + '</div>';
        loteEl.closest('.f').after(div);
      }
    }, 100);
  };
})();
...
// ── Preencher campos do cliente no módulo inline (usado pelo orcEditar) ───
window._TI_preencherCliente = function(pend) {
  ...
  var clienteFields = {
    'iCli':       pend.cli  || '',
    'iTel':       pend.tel  || '',
    'iCemiterio': pend.cemi || '',
    'iCidade':    pend.cid  || '',
    'iQuadra':    pend.quad || '',
    'iLote':      pend.lote || '',
    'iObs':       pend.obs  || ''
  };
  Object.keys(clienteFields).forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.value = clienteFields[id];
  });
  if (Array.isArray(pend.fal) && pend.fal.length > 0) {
    SEL.falecidos = JSON.parse(JSON.stringify(pend.fal));
    if (typeof renderFalecidos === 'function') renderFalecidos();
  }

  // ── 2. Configurações SEL ──────────────────────────────────────────────
  if (pend._sel) {
    var s = pend._sel;
    if (s.tipoServ)  { SEL.tipoServ  = s.tipoServ;  if (typeof selTipoServ === 'function') selTipoServ(s.tipoServ); }
    if (s.matId)     { SEL.matId     = s.matId;     selMat(s.matId); }
    if (s.acabamento){ SEL.acabamento = s.acabamento; if (typeof selAcab === 'function') selAcab(s.acabamento); }
  ...
  // ── 3. Preset e tipo ──────────────────────────────────────────────────
  if (pend.preset) {
    SEL.preset = pend.preset;
    if (typeof aplicarPreset === 'function') aplicarPreset(pend.preset);
  }
  ...
  if (pend._dims) {
    ...
    setVal('mE',          dm.E);
    setVal('mGav',        dm.N);
    setVal('mDisp',       dm.disp);
    setVal('mAe',         dm.Ae);
    setVal('mAb',         dm.Ab);
    setVal('mHcomp',      dm.Hc);
    setVal('mHlaje',      dm.Hl);
    setVal('mLapW',       dm.LapW);
    setVal('mLapH',       dm.LapH);
    setVal('mAvRodape',   dm.AvRod);
    setVal('mAlturaFinal',dm.altFinal);
    if (SEL.rebaixo && dm.AvRod !== undefined) SEL.rebaixo.avRodape = dm.AvRod;
  } else if (pend.r && pend.r.d) {
    // Fallback: usar r.d para recompor as dimensões
    var d2 = pend.r.d;
    var setV2 = function(id, v) {
      var el = document.getElementById(id);
      if (el && v !== undefined && v !== null) el.value = v;
    };
    setV2('mC', d2.C_cm); setV2('mL', d2.L_cm); setV2('mE', d2.E);
    setV2('mGav', d2.N);  setV2('mDisp', d2.disp);
    setV2('mAe', d2.Ae_cm); setV2('mAb', d2.Ab_cm);
    setV2('mHcomp', d2.Hc_cm); setV2('mHlaje', d2.Hl_cm);
    setV2('mLapW', d2.LapW_cm); setV2('mLapH', d2.LapH_cm);
    if (d2.AvRod !== undefined) { setV2('mAvRodape', d2.AvRod); SEL.rebaixo.avRodape = d2.AvRod; }
  }

  // ── 5. Recalcular e atualizar UI ──────────────────────────────────────
  if (typeof buildPosicaoPresets === 'function') buildPosicaoPresets();
  if (typeof atualizarTampasUI  === 'function') atualizarTampasUI();
  if (typeof renderPecasTogList === 'function') renderPecasTogList();
  _TI_calcular();
};

// ── Compartilhar PDF via Web Share API (Android/iOS) ─────────────────────
window.compartilharPDF = function() {
  if (!pendOrc) { toast('Gere um orçamento primeiro', true); return; }
  var pBody = document.getElementById('pBody');
  if (!pBody || !pBody.innerHTML.trim()) { toast('Recalcule antes de compartilhar', true); return; }
  ...
  if (navigator.canShare && navigator.share) {
    ...
    var nomeCliente = (pendOrc.cli||'cliente').replace(/[^a-zA-Z0-9]/g,'_');
    var file = new File([blob], 'Orcamento_'+nomeCliente+'.html', { type: 'text/html' });
    if (navigator.canShare({ files:[file] })) {
      navigator.share({ files:[file], title:'Orçamento — '+(pendOrc.cli||''), text:'Orçamento HR Mármores e Granitos' })
        .catch(function(e){ if(e.name!=='AbortError') _abrirJanelaPDF(pBody.innerHTML); });
      return;
    }
  }
...
// Expõe leitura do catálogo local — evita que código externo acesse
// localStorage para obter pedras; retorna sempre o array vivo de CFG.pedras
// (inclui pedras adicionadas via confirmarAddPedra no ciclo atual)
window.tumGetPedrasLocais = function() {
  return CFG.pedras.slice(); // cópia rasa — seguro para leitura
};
