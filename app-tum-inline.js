...
function calcularFull() {
  ...
  if (SEL.rebaixo && (SEL.rebaixo.usinagem || SEL.rebaixo.lajeVedante)) {
    ...
    // Área de cada abertura (onde a laje vedante se encaixa)
...
function atualizarAreaUtil() {
  ...
  if (infoLaje && r.m2_laje_ved > 0) {
    infoLaje.textContent = r.m2_laje_ved.toFixed(3) + ' m² · ' +
      r.m2_laje_ved.toFixed(3) + ' m² × R$' + (SEL.rebaixo.vendaLaje || 200) + '/m² = R$ ' +
      r.venda_laje_ved.toFixed(2);
  } else if (infoLaje) {
    var td = getTampasDims();
    infoLaje.textContent = td.nTotal + ' lajes de ' +
