...
(function () {
  ...
  function _renderModal() {
    ...
    var prevTextarea = (document.getElementById('im-cola') || {}).value || '';
    ...
    h += '<div style="font-size:.62rem;color:var(--t3);">Cole os dados do anúncio e envie as fotos</div>';
    h += '</div>';
    h += '<button onclick="_imFecharModal()" style="background:none;border:none;color:var(--t3);'
    ...
    h += '<textarea id="im-cola" rows="2" placeholder="Ex: Cuba de Apoio Redonda Inox 35cm - R$ 189,90" '
       ...
       + 'padding:9px 11px;color:var(--tx);font-size:.78rem;box-sizing:border-box;outline:none;'
       + 'resize:vertical;margin-bottom:10px;font-family:inherit;">' + _esc(prevTextarea) + '</textarea>';

    // ─── Upload de fotos ──────────────────────────────────────
    h += '<div style="font-size:.6rem;color:var(--t3);margin-bottom:5px;">Fotos do produto</div>';
    h += '<label style="display:block;width:100%;padding:13px;border-radius:10px;'
       + 'border:1px dashed var(--gold3);background:var(--gdim);color:var(--gold2);'
    ...
    if (_im.fotos.length) {
      ...
      _im.fotos.forEach(function(foto, i) {
        ...
        h += '<img src="' + foto + '" onclick="_imSelecionarFoto(' + i + ')" '
           + 'style="width:58px;height:58px;object-fit:cover;border-radius:8px;cursor:pointer;'
           + 'border:2px solid ' + (sel ? 'var(--gold2)' : 'transparent') + ';box-sizing:border-box;">';
        h += '<button onclick="event.stopPropagation();_imRemoverFoto(' + i + ')" '
           + 'style="position:absolute;top:-6px;right:-6px;width:18px;height:18px;border-radius:50%;'
           + 'background:#c0392b;color:#fff;border:none;font-size:.65rem;line-height:1;cursor:pointer;'
           + 'display:flex;align-items:center;justify-content:center;">✕</button>';
        h += '</div>';
      });
      h += '</div>';
    } else {
      h += '<div style="text-align:center;padding:18px 0;color:var(--t3);font-size:.72rem;'
         + 'margin-bottom:14px;opacity:.7;">Nenhuma foto enviada ainda</div>';
    }

    // ─── Nome editável ────────────────────────────────────────
    h += '<div style="font-size:.6rem;color:var(--t3);margin-bottom:3px;">Nome do produto</div>';
    h += '<input id="im-nome" type="text" value="' + _esc(prevNome) + '" placeholder="ex: Cuba de Apoio Redonda Inox" '
    ...
    h += '<input id="im-dim" type="text" value="' + _esc(prevDim) + '" placeholder="ex: 35×35cm" '
       + 'style="width:100%;background:var(--s2);border:1px solid var(--bd2);border-radius:9px;'
       + 'padding:9px 11px;color:var(--tx);font-size:.78rem;box-sizing:border-box;outline:none;margin-bottom:12px;">';
    ...
    h += '<div style="flex:1;">'
       + '<div style="font-size:.58rem;color:var(--t3);margin-bottom:3px;">Preço de custo (R$)</div>'
       + '<input id="im-custo" type="text" inputmode="decimal" value="' + _esc(prevCusto) + '" placeholder="0,00" '
       + 'oninput="_imRecalcular()" '
       + 'style="width:100%;background:var(--s2);border:1px solid var(--bd2);border-radius:8px;'
       + 'padding:7px 9px;color:var(--tx);font-size:1.05rem;font-weight:700;'
       + 'box-sizing:border-box;outline:none;">'
       + '</div>';
    h += '<div style="width:80px;">'
       + '<div style="font-size:.58rem;color:var(--t3);margin-bottom:3px;">Margem %</div>'
       + '<input id="im-margem" type="number" min="0" max="300" step="1" value="' + _im.margem + '" '
       + 'oninput="_imRecalcular()" '
       + 'style="width:100%;background:var(--s2);border:1px solid var(--bd2);border-radius:8px;'
    ...
    h += '<div style="display:flex;justify-content:space-between;align-items:baseline;">'
       ...
       + '<div id="im-pr-venda" style="font-family:\'Cormorant Garamond\',serif;'
       + 'font-size:1.6rem;font-weight:700;color:var(--gold2);">'
       + _fmt(_precoVenda(parseFloat((prevCusto||'0').replace(',', '.')) || 0, _im.margem)) + '</div>'
       + '</div>';
    h += '</div>';

    h += '<div style="font-size:.58rem;color:var(--t3);margin-bottom:14px;'
  ...
  function _salvar() {
    ...
    if (!nome) {
      _imAlerta('⚠️ Digite o nome do produto antes de salvar.');
    ...
    var cat   = _im.cat;
    var foto  = _im.fotos.length ? _im.fotos[_im.selIdx] : '';
